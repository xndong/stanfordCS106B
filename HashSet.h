...
struct HashNode
{
     ...
     * constructor,destructor可以在类的外部(eg .cppw file中)实现，也可以在类的内部实现(eg这里)。
...
private:
    ...
    //member functions            Note that it not pointer of pointer, and it array of pointer.
    int hashCode(int value) const;
};
#endif
/**
 * HashSet的核心数据结构是一个bucket,这个buckets是一个vector,
 * vector的元素是HashNode *类型，HashNode *不就和我们以前学的LinkedList里ListNode *一样么~
 * 所以vector的元素是一个LinkedList,这个LinkedList的动机也是解决hash collision vs using probing.
*/
 ...
 * 书的思路十分清晰：从简单到复杂，把知识一点点引进来，带你一点点升级，写法越来越越拜托naive的方式。
*/